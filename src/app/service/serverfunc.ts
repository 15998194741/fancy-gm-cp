...
async function findAll(gameName:string,platform:string,channelNum:string,versionId:string,ip:string):Promise<findAllData[]>{
    let sql = `
    with qwe as ( select id from gm_game  where game_name = '${gameName}' and status = 1 ),
            ...
            zxc as (select a.* ,'${channelNum}' as channelNum from gm_server a,asd  where  plaform @> '"${platform}"' and  a.channel @> concat('["' ,asd.channel,'"]' )::jsonb and status = 1 and a.clientshow = '1'),
            ert as  (select case when type = '测试' then '1' when type ='正式' then '0' end  as test from (select jsonb_array_elements_text(is_show_type) as type from gm_client,qwe where game_id::int = qwe.id  and version_id =  '${versionId}' and status = '1' ) a )
